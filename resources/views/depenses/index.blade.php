            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="px-2 py-1 text-xs rounded bg-red-100 text-red-800">{{ $depense->motif }}</span>
                    </td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <button onclick="confirmDeleteDepense({{ $depense->id }})" 
                ...
                @empty
                <tr>
                    <td colspan="5" class="px-6 py-4 text-center text-gray-500">Aucune dépense enregistrée</td>
                </tr>
                @endforelse
            </tbody>
        </table>
...
<div id="modal-add-depense" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg w-full max-w-2xl mx-4 max-h-screen overflow-y-auto">
        ...
        <form action="{{ route('depenses.store') }}" method="POST" class="p-6">
            @csrf
...
<!-- Modal de confirmation de suppression dépense -->
<div id="modal-delete-depense" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg w-full max-w-md mx-4">
        ...
        <div class="p-6 border-t flex justify-end space-x-3">
            <button onclick="document.getElementById('modal-delete-depense').classList.add('hidden')" 
                    class="px-4 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50">
                Annuler
            </button>
            <form id="delete-depense-form" method="POST" action="">
                @csrf
                @method('DELETE')
                <button type="submit" class="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700">
                    Supprimer
...
function confirmDeleteDepense(id) {
    const form = document.getElementById('delete-depense-form');
    form.action = '/depenses/' + id;
    document.getElementById('modal-delete-depense').classList.remove('hidden');
}
</script>
@endsection